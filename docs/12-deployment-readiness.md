...
- KV limiting is best effort: it fails open and KV has no atomic increment, so concurrent verified submissions from one IP can briefly exceed the limit. Quota is consumed only after Turnstile succeeds (unverified requests cannot exhaust a shared IP), and counters use fixed ten-minute windows so a burst cannot extend its own expiry. Do not promise a strict five-per-ten-minutes limit. Add upstream abuse controls if strict limits are needed and monitor KV errors.
