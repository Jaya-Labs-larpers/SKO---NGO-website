...
export const sanityApiVersion = str(env.PUBLIC_SANITY_API_VERSION) || '2026-01-01';
 ...
 */
export const hasSanity = sanityProjectId.length > 0;

/* --- Site ----------------------------------------------------------------- */

export const siteUrl = (str(env.PUBLIC_SITE_URL) || 'http://localhost:4321').replace(/\/+$/, '');
...
export function reportConfig(): void {
  const mode = hasSanity ? `Sanity project ${sanityProjectId}/${sanityDataset}` : 'LOCAL FIXTURES';
  const notes: string[] = [];
  if (!hasSanity) notes.push('PUBLIC_SANITY_PROJECT_ID is not set — using src/lib/content/fixtures.ts');
  if (!turnstileSiteKey) notes.push('PUBLIC_TURNSTILE_SITE_KEY is not set — forms render without the widget');
  if (!cfAnalyticsToken) notes.push('PUBLIC_CF_ANALYTICS_TOKEN is not set — analytics beacon omitted');
